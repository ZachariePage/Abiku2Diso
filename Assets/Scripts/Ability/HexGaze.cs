...
public class HexGaze : DamagingAbility
{
    int normalAmount;
    int advantageAmount;
    public HexGaze(ISpellCaster caster, GridActor actor, AbilityTemplateSO template, int range, 
        TargetingStrategySO direction, TargetTypeStrategySO targetAllowed, int numberOfTargets, ElementSO element,
        HexGazeTemplateSO config, int normalAmount, int advantageHexAmount)
    ...
    public override IEnumerator Execute(Action onComplete)
    {
        ...
        if (target != null)
        {
            ...
            witch.ApplyHex(target, normalAmount);
        }
